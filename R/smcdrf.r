...
smcdrf_test_3 <- function(target,
        ...
        for (iteration in 1:nIter) {
            ...
            p_lambda <- p_lambda +
                geom_density(
                    data = cbind(parameters, w),
                    aes(x = lambda, weight = w, fill = paste0("Iter-", iteration), color = paste0("Iter-", iteration)), alpha = 0.2, size = 2
                )
            p_mu <- p_mu +
                geom_density(
                    data = cbind(parameters, w),
                    aes(x = mu, weight = w, fill = paste0("Iter-", iteration), color = paste0("Iter-", iteration)), alpha = 0.2, size = 2
                )
            ########################################################  TEST - END
        }
        ######################################################  TEST - BEGIN
